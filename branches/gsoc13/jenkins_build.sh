...
if [ "$1" == "status" ]; then
	...
	for b in linux macosx cygwin mingw msvc ; do
		echo $build/$b
		curl --silent $JENKINS/job/Exiv2-$build/label=$b/lastBuild/consoleText | tee $tmp | grep -E -e SVN_[A-Z]+= -e JOB_NAME -e BUILD_ID -e Finished -e GMT $@ ;
